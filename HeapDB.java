		...
		if (blockNum > 1 && blockNum <= bf.getLastBlockIndex()) {
			...
			if (recNum >= 0) {
				...
				while(i<indexes.length)
				{
					...
					{
						Field f = rec.get(i);
						IntField temp = (IntField) f;
						int val = temp.getValue();
						indexes[i].insert(val, blockNum);
					}
					i++;
				}
	...
	public List<Record> lookup(String fname, int key) {
		...
		List<Record> result = new ArrayList<Record>();
				...
				else 
				{
					...
					{
						// calling lookupInBlock to get the actual records
						List<Record> temp = lookupInBlock(fieldNum, key, i);
						// adding records into "result'
						result.addAll(temp);
					}
				}
